...
PROGMEM const fixedEvent_t programEvent[] =
{
    ...
    {dowMonday, 18, 30, OFF},
    {dowTuesday, 12, 00, ON},
    {dowTuesday, 12, 30, OFF},
    {dowWednesday, 6, 32, ON},
    {dowWednesday, 9, 12, OFF},
...
void EventManager::PopFirstEvent(void)
{
    event_t * current;
...
void EventManager::RestoreDay(timeDayOfWeek_t input_day)
{
    ...
    dbg("Number of event before %d", EventNumber());
    ...
    */
    ...
    // add days
    time_day += ((input_day - weekday() + 7) % 7 )*SECS_PER_DAY;
    /*
...
void EventManager::PrintAllEvent(void)
{
    event_t * ptr;

    // DEBUG purpose
    serial_printf_f(F("Event List:"));

    ptr = first;
    while (ptr)
    {
	serial_printf_f(F("Event %02d, %02d.%02d.%04d-%02d:%02d:%02d, act %d"),
